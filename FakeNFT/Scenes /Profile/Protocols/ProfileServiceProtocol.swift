...
protocol ProfileServiceProtocol {
    ...
    /// - Parameters:
    ///    - profileDto: DTO с обновленными данными профиля, которые необходимо сохранить на сервере
    ///    - completion: Вызывается после завершения обновления профиля
    ...
    /// Обновить лайки профиля на сервере
    /// - Parameters:
    ///    - likesDto: DTO с обновленным массивом лайков, которые необходимо сохранить на сервере
    ///    - completion: Вызывается после завершения обновления профиля
    ///         - При успехе (`.success`) возвращает обновленный объект `Profile`
    ///         - При ошибке (`.failure`) возвращает `Error`
